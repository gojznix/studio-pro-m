  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center p-4 rounded-lg bg-white/10 border border-white/20">
        <h3 className="text-white/90 font-medium mb-2">Status Samodejnega predvajanja</h3>
        <div
        ...
        >
          ...
          <span className="text-sm font-medium">{isAutoPlaying ? "Aktivno" : "Zaustavljeno"}</span>
        </div>
      </div>

      {/* Advertisement Status */}
      <div className="space-y-2">
        <h3 className="text-xl font-semibold text-white/90">Metrike Oglasa</h3>
        <div className="max-h-[200px] overflow-y-auto pr-1 scrollbar-thin space-y-2">
          {advertisements.map((ad) => {
            ...
            return (
              <div key={ad.id} className="p-3 rounded-lg backdrop-blur-md bg-white/10 border border-white/20">
      ...
      <div className="space-y-2">
        ...
        <div className="max-h-[320px] overflow-y-auto pr-1 scrollbar-thin">
          {songs
            .filter((song) => (song.effectiveRating ?? song.rating ?? 0) >= 8.5)
            .slice(0, 10)
            .map((song) => {
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex items-center space-x-1 bg-black/30 px-2 py-1 rounded-full">
                      ...
                      <span className="text-white font-bold text-sm">
                        {(song.effectiveRating ?? song.rating ?? 0).toFixed(1)}
                      </span>
                    </div>
                  </div>
                </div>
